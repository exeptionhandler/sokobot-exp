    ...
    public void execute(CommandEvent event) {
        ...
        if (args.length > 0) {
            if (!member.hasPermission(Permission.ADMINISTRATOR)) {
                ...
                event.reply(user.getAsMention() + ", lo siento, pero no tienes permiso para usar este comando.");
                return;
            }
            String newPrefix = args[0].toLowerCase();
            if (newPrefix.length() > 1) {
                Bot.debug("Failed to change prefix of " + guild.getName() + " (" + guild.getId() + "): length");
                event.reply(user.getAsMention() + ", ¡el prefijo solo debe tener un car\u00e1cter!");
                return;
            }
            Bot.setPrefix(guild, newPrefix);
            Bot.debug("Successfully changed server prefix of " + guild.getName() + " (" + guild.getId() + ") to: "
                              + newPrefix);
            event.reply("Prefijo cambiado correctamente a ``" + newPrefix + "``.");
            return;
        }
        event.reply(user.getAsMention() + ", por favor use `" + Bot.getPrefix(guild)
                            + "prefix <nuevo prefijo>` para establecer un prefijo de servidor.");
    }
}
