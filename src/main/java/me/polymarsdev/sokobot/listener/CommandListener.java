...
public class CommandListener extends ListenerAdapter {
    private static final ArrayList<String> commandsNoPrefix = new ArrayList<>(
            Arrays.asList("w", "a", "s", "d", "arriba", "izquierda", "abajo", "derecha", "r", "mr"));
    ...
    private void sendInvalidPermissionsMessage(User user, TextChannel channel) {
        if (channel.canTalk()) {
            ...
            channel.sendMessage(user.getAsMention() + ", No tengo suficientes permisos para trabajar correctamente.\nAsegurate "
                                        + "de que tenga los siguientes permisos:" + requiredPermissionsDisplay
                                        + "\nSi crees que esto es"
                                        + "un error , póngase en contacto con un administrador del servidor.").queue();
        }
    }
}