...
public class Bot {
    ...
    public static void main(String[] args) throws LoginException {
        String token = null;
        try {
            File tokenFile = Paths.get("token.env").toFile();
            if (!tokenFile.exists()) {
                System.out.println("[ERROR] Could not find token.env file");
                System.out.print("Please paste in your bot token: ");
                Scanner s = new Scanner(System.in);
                token = s.nextLine();
                System.out.println();
                System.out.println("[INFO] Creating token.env - please wait");
                if (!tokenFile.createNewFile()) {
                    System.out.println(
                            "[ERROR] Could not create token.env - please create this file and paste in your token"
                                    + ".");
                    s.close();
                    return;
                }
        ...
        builder.setActivity(Activity.playing("@Sokobot Espa\u00f1ol  para informaci\u00f3n!"));
    ...
    private static void processCommand(String cmd) {
        if (cmd.equalsIgnoreCase("help")) {
            System.out.println("Comandos:\nstop - Apaga el bot y sale del programa\ndebug - Alternar el modo de depuración");
            return;
        }
        if (cmd.equalsIgnoreCase("debug")) {
            ...
            Bot.debug("Aseg\u00farese de desactivar el modo de depuraci\u00f3n despu\u00e9s de recopilar la informaci\u00f3n necesaria.");
            return;
        }
        ...
        System.out.println("Comando Desconocido. Porfavor use \"help\" para una lista de commandos.");
    }
